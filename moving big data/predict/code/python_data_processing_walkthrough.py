...
source_path = "../Stocks/"
save_path = "../Output/"
index_file_path = '../CompanyNames/top_companies.txt'

# # EC2 file directories
# Do not change these paths
# source_path = "/home/ec2-user/my-s3-bucket/Stocks/"
# save_path = "/home/ec2-user/my-s3-bucket/Output/"
# index_file_path = "/home/ec2-user/my-s3-bucket/CompanyNames/top_companies.txt"
...
def data_processing(file_paths, output_path):
    ...
    save_table(combined_csv, output_path, 'historical_stock_data', header=True)
...
def test_data_processing(file_paths, output_path):
    sdf = pd.DataFrame()
    sdf.to_csv(output_path+"historical_stock_data.csv")
    for file_path in file_paths:
        try:
            df = pd.read_csv(file_path) 
            sdf[["stock_date"]]=df[["Date"]]
            sdf[["open_value"]]=df[["Open"]]
            sdf[["high_value"]]=df[["High"]]
            sdf[["low_value"]]=df[["Low"]]
            sdf[["close_value"]]=df[["Close"]]
            sdf[["volume_traded"]]=df[["Volume"]]
            sdf[["company_name"]]=file_path[10:-7]
            #sdf[["daily_percent_change"]]=((df["Close"]-df["Open"])/df["Open"])*100
            #sdf[["value_change"]]=df["Close"]-df["Open"]
            # percentage_change=((df["Close"]-df["Open"])/df["Open"])*100
            # value_change = df["Close"]-df["Open"]
            # sdf[["daily_percent_change"]]=percentage_change
            # sdf[["value_change"]]=value_change
            
            sdf.to_csv(output_path+"historical_stock_data.csv", mode='a', header=False)
        except:
            pass
        break
    print(sdf) 
